  ...
  const deleteAccount = useCallback(async () => {
    ...
    if (session?.access_token) {
      try {
        ...
        // If the function call fails for any reason, still fall back to
        // wiping the user's data so "Delete account" isn't a no-op.
...
@media (max-width: 640px) {
  ...
  .board, .kanban-board, .status-columns {
    display: flex; overflow-x: auto; -webkit-overflow-scrolling: touch;
    scroll-snap-type: x mandatory; gap: 10px;
  }
  .board-column, .status-column, .kanban-column {
    min-width: 84vw; scroll-snap-align: start; flex: 0 0 auto;
  }
