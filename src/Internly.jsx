...
function InternlyApp() {
  ...
  const changeLogo = useCallback(async (id) => {
    setLogoId(id);
    try {
      await sbSetData("settings", { logoId: id, favoriteGames, showAllFilter, highContrastNav });
    } catch (e) {
      // non-fatal - logo choice just won't persist
    }
  }, [favoriteGames, showAllFilter, highContrastNav]);
  ...
  const toggleFavoriteGame = useCallback((title) => {
    setFavoriteGames((current) => {
      ...
      sbSetData("settings", { logoId, favoriteGames: next, showAllFilter, highContrastNav }).catch(() => {});
      return next;
    });
  }, [logoId, showAllFilter, highContrastNav]);

  const setShowAllFilter = useCallback((value) => {
    setShowAllFilterState(value);
    sbSetData("settings", { logoId, favoriteGames, showAllFilter: value, highContrastNav }).catch(() => {});
  }, [logoId, favoriteGames, highContrastNav]);

  const setHighContrastNav = useCallback((value) => {
    setHighContrastNavState(value);
    sbSetData("settings", { logoId, favoriteGames, showAllFilter, highContrastNav: value }).catch(() => {});
  }, [logoId, favoriteGames, showAllFilter]);
...
function SettingsPage({ logoId, onChangeLogo, account, onUpdateAccount, onSignOut, onDeleteAccount, gpaSettings, onUpdateGpaSettings, showAllFilter, onSetShowAllFilter, highContrastNav, onSetHighContrastNav }) {
