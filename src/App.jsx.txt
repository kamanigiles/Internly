...
async function sbSignUp(email, password, name) {
  ...
  if (json.access_token) sbSaveSession(json);
...
async function sbGetData(key) {
  ...
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/app_data?user_id=eq.${userId}&key=eq.${encodeURIComponent(key)}&select=value`,
    { headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${session.access_token}` } }
  );
...
function LoginFlow({ onLogin, onBack, logoId, defaultEmail }) {
  ...
  return (
    <div className="auth-stage">
      <div className="auth-card">
        ...
        <button className="btn btn-primary auth-submit" onClick={handleLogin} disabled={checking}>
...
function AuthFlow({ onComplete, onLogin, logoId }) {
  ...
  const validateSignup = () => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(signup.email.trim())) return "Enter a valid email address.";
        ...
        {step === 1 ? (
          <>
            ...
            <label className="auth-field">
              ...
              <div className="input-with-icon">
                <User size={13} />
                <input value={signup.name} onChange={setSignupField("name")} placeholder="Jane Doe" autoFocus />
              </div>
            </label>
            <label className="auth-field">
              Email
              <div className="input-with-icon">
                <Mail size={13} />
                <input type="email" value={signup.email} onChange={setSignupField("email")} placeholder="jane@school.edu" />
              </div>
            </label>
            <label className="auth-field">
              Password
          ...
          <>
            ...
            <div className="auth-field-row">
              <label className="auth-field">
                School
                <input value={profile.school} onChange={setProfileField("school")} placeholder="e.g. University of Maryland" autoFocus />
              </label>
              <label className="auth-field">
                Major
...
function InternlyApp() {
  ...
  const [quickSaveDraft, setQuickSaveDraft] = useState({ company: "", link: "" });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          if (parsed) {
            setApps(normalizeApps(parsed));
          } else {
            const seed = seedData();
            setApps(seed);
            await sbSetData("applications", seed);
          }
        }
      } catch (e) {
        if (!cancelled) setApps(seedData());
      } finally {
  ...
  const changeLogo = useCallback(async (id) => {
    setLogoId(id);
    try {
      await sbSetData("settings", { logoId: id, favoriteGames });
    } catch (e) {
  ...
  const toggleFavoriteGame = useCallback((title) => {
    setFavoriteGames((current) => {
      ...
      sbSetData("settings", { logoId, favoriteGames: next }).catch(() => {});
      return next;
    });
  }, [logoId]);
    ...
    return (
      <div className="internly-root">
        ...
        {manualLoginMode ? (
          ...
          <AuthFlow onComplete={completeSignup} onLogin={() => setManualLoginMode(true)} logoId={logoId} />
        )}
      </div>
    );
  }
      ...
      <div className="semester-head">
        ...
        <div className="semester-head-actions">
          {confirmDelete ? (
            ...
            <>
              <button className="icon-btn" onClick={() => setQuickView(true)} title="Quick view">
                <Eye size={14} />
              </button>
              <button className="icon-btn" onClick={() => setConfirmDelete(true)} title="Remove semester">
                <Trash2 size={14} />
              </button>
            </>
          )}
            ...
            <div className="course-table">
              ...
              {semester.courses.map((c) => (
                <div className="course-row" key={c.id}>
                  ...
                  <button className="icon-btn" onClick={() => removeCourse(c.id)} title="Remove course">
                    <Trash2 size={12} />
                  </button>
                </div>
              ))}
            ...
            {semester.courses.length === 0 ? (
              ...
              semester.courses.map((c) => (
                <div className="qv-course-row" key={c.id}>
                  ...
                  <span className="qv-course-meta">{c.credits || "—"} cr · {c.category}</span>
  ...
  return (
    <div className="settings-section">
      ...
      {expanded && (
        <>
          <p className="settings-sub">
            Not every school weighs letter grades the same way — set your own grade-to-GPA-point values here.
          </p>
          <div className="grade-scale-table">
            ...
            {draft.map((row, i) => (
              <div className="grade-scale-row" key={i}>
                ...
                <input value={row.points} onChange={(e) => updateRow(i, { points: e.target.value })} placeholder="4.0" inputMode="decimal" />
                <button className="icon-btn" onClick={() => removeRow(i)} title="Remove grade">
                  <Trash2 size={12} />
                </button>
              </div>
            ))}
    ...
    <div className="settings-section">
      ...
      <div className="grad-progress">
        ...
        <span className="grad-progress-label">
          {cumulative.credits} of {degreeCredits || "—"} credits ({pctComplete.toFixed(0)}%)
        </span>
      </div>
  ...
  return (
    <div className="academics-page">
      <div className="stats-row academics-stats-row-4">
        <StatCard label="Overall GPA" value={cumulativeStr === null ? "—" : cumulativeStr} icon={<GraduationCap size={16} />} />
    ...
    <div className="card" onClick={onQuickView} role="button" tabIndex={0}>
      <div className="card-top">
        ...
        <div className="card-actions">
          <button className="icon-btn" onClick={(e) => { e.stopPropagation(); onQuickView(); }} title="Quick view">
            <Eye size={13} />
          </button>
          <button className="icon-btn" onClick={(e) => { e.stopPropagation(); onEdit(); }} title="Edit">
            <Pencil size={13} />
          </button>
...
function ScholarshipsPage({ scholarships, loading, onPersist }) {
  const [filter, setFilter] = useState("All");
  const [query, setQuery] = useState("");
  ...
  return (
    <div className="scholarships-page">
      ...
      <div className="controls-row">
        ...
        <div className="chip-filters">
          {["All", ...SCHOLARSHIP_STATUSES].map((s) => (
            <button
              key={s}
              className={`filter-chip ${filter === s ? "filter-chip-active" : ""}`}
              onClick={() => setFilter(s)}
        ...
        <div className="modal-body">
          <div className="field-row">
            <label>
              ...
              <input value={draft.organization} onChange={set("organization")} placeholder="e.g. Humane Society" autoFocus />
            </label>
            <label>
              Role / activity
...
function VolunteerPage({ volunteering, loading, onPersist }) {
  const [filter, setFilter] = useState("All");
  const [query, setQuery] = useState("");
  const [draft, setDraft] = useState(null);
  const [isEditing, setIsEditing] = useState(false);
...
function DashboardPage({ account, apps, semesters, gpaSettings, scholarships, onNavigate }) {
  ...
  const overallGpa = cumulative.gpa === null ? "—" : cumulative.gpa.toFixed(2);
...
function SettingsPage({ logoId, onChangeLogo, account, onUpdateAccount, onSignOut, gpaSettings, onUpdateGpaSettings }) {
      ...
      {account && (
        <div className="settings-section">
          ...
          <div className="field-row">
            <label>
              School
              <input value={profile.school} onChange={setField("school")} placeholder="e.g. University of Maryland" />
            </label>
            <label>
              Major
...
const US_LOCATIONS = [
  "Remote",
  "New York, NY", "Brooklyn, NY", "Queens, NY", "Bronx, NY", "Staten Island, NY", "Buffalo, NY", "Albany, NY", "Rochester, NY", "Syracuse, NY", "Yonkers, NY", "New Rochelle, NY", "Ithaca, NY", "Binghamton, NY", "White Plains, NY", "Poughkeepsie, NY", "Schenectady, NY", "Utica, NY", "Niagara Falls, NY", "Troy, NY", "Long Island, NY", "Hempstead, NY", "New Paltz, NY", "Saratoga Springs, NY", "Plattsburgh, NY", "Geneva, NY",
  "Los Angeles, CA", "San Francisco, CA", "San Diego, CA", "San Jose, CA", "Sacramento, CA", "Oakland, CA", "Irvine, CA", "Palo Alto, CA", "Mountain View, CA", "Long Beach, CA", "Fresno, CA", "Bakersfield, CA", "Anaheim, CA", "Santa Ana, CA", "Riverside, CA", "Santa Barbara, CA", "Santa Cruz, CA", "Berkeley, CA", "Pasadena, CA", "Fremont, CA", "Santa Monica, CA", "Palm Springs, CA", "San Luis Obispo, CA", "Chula Vista, CA", "Stockton, CA", "Burbank, CA", "Glendale, CA", "Torrance, CA", "Concord, CA", "Modesto, CA", "Napa, CA", "Sonoma, CA", "Malibu, CA", "Redwood City, CA", "Sunnyvale, CA", "Cupertino, CA", "Davis, CA", "Chico, CA", "Ventura, CA", "Carlsbad, CA", "Temecula, CA", "Eureka, CA", "Redding, CA",
  "Chicago, IL", "Springfield, IL", "Naperville, IL", "Peoria, IL", "Rockford, IL", "Champaign, IL", "Evanston, IL", "Aurora, IL", "Joliet, IL", "Bloomington, IL", "Decatur, IL", "Elgin, IL", "Schaumburg, IL", "Waukegan, IL", "DeKalb, IL",
  "Houston, TX", "Austin, TX", "Dallas, TX", "San Antonio, TX", "Fort Worth, TX", "El Paso, TX", "Plano, TX", "Corpus Christi, TX", "Arlington, TX", "Lubbock, TX", "Frisco, TX", "McKinney, TX", "College Station, TX", "Waco, TX", "Galveston, TX", "Irving, TX", "Denton, TX", "Round Rock, TX", "Amarillo, TX", "Abilene, TX", "Beaumont, TX", "Laredo, TX", "Killeen, TX", "Tyler, TX", "Midland, TX", "Odessa, TX", "San Marcos, TX", "Georgetown, TX", "The Woodlands, TX", "Sugar Land, TX",
  "Phoenix, AZ", "Tucson, AZ", "Scottsdale, AZ", "Mesa, AZ", "Tempe, AZ", "Chandler, AZ", "Flagstaff, AZ", "Gilbert, AZ", "Glendale, AZ", "Peoria, AZ", "Yuma, AZ", "Sedona, AZ",
  "Philadelphia, PA", "Pittsburgh, PA", "Harrisburg, PA", "Allentown, PA", "Erie, PA", "Scranton, PA", "State College, PA", "Bethlehem, PA", "Lancaster, PA", "Reading, PA", "York, PA", "Wilkes-Barre, PA", "Altoona, PA", "Chester, PA", "West Chester, PA", "Lehigh Valley, PA",
  "Baltimore, MD", "Annapolis, MD", "Rockville, MD", "Bethesda, MD", "Silver Spring, MD", "Columbia, MD", "Towson, MD", "Frederick, MD", "Gaithersburg, MD", "Hagerstown, MD", "College Park, MD", "Ellicott City, MD", "Salisbury, MD", "Bowie, MD", "Germantown, MD", "Cumberland, MD", "Ocean City, MD", "Laurel, MD", "Waldorf, MD",
  "Washington, DC",
  "Boston, MA", "Cambridge, MA", "Worcester, MA", "Springfield, MA", "Lowell, MA", "Somerville, MA", "Newton, MA", "Amherst, MA", "Northampton, MA", "New Bedford, MA", "Brookline, MA", "Salem, MA", "Quincy, MA", "Plymouth, MA", "Lexington, MA", "Framingham, MA",
  "Atlanta, GA", "Savannah, GA", "Augusta, GA", "Athens, GA", "Columbus, GA", "Macon, GA", "Marietta, GA", "Alpharetta, GA", "Roswell, GA", "Valdosta, GA", "Decatur, GA", "Sandy Springs, GA",
  "Miami, FL", "Orlando, FL", "Tampa, FL", "Jacksonville, FL", "Fort Lauderdale, FL", "Tallahassee, FL", "St. Petersburg, FL", "Gainesville, FL", "Sarasota, FL", "West Palm Beach, FL", "Boca Raton, FL", "Naples, FL", "Pensacola, FL", "Fort Myers, FL", "Key West, FL", "Daytona Beach, FL", "Melbourne, FL", "Coral Gables, FL", "Clearwater, FL", "Hialeah, FL", "Kissimmee, FL", "Boynton Beach, FL", "Ocala, FL",
  "Seattle, WA", "Bellevue, WA", "Spokane, WA", "Tacoma, WA", "Olympia, WA", "Everett, WA", "Redmond, WA", "Vancouver, WA", "Bellingham, WA", "Kirkland, WA", "Renton, WA", "Yakima, WA", "Pullman, WA",
  "Denver, CO", "Boulder, CO", "Colorado Springs, CO", "Fort Collins, CO", "Aurora, CO", "Grand Junction, CO", "Pueblo, CO", "Greeley, CO", "Vail, CO", "Durango, CO",
  "Portland, OR", "Eugene, OR", "Salem, OR", "Bend, OR", "Corvallis, OR", "Medford, OR", "Beaverton, OR", "Hillsboro, OR", "Ashland, OR",
  "Detroit, MI", "Ann Arbor, MI", "Grand Rapids, MI", "Lansing, MI", "Flint, MI", "East Lansing, MI", "Kalamazoo, MI", "Dearborn, MI", "Troy, MI", "Sterling Heights, MI", "Traverse City, MI", "Saginaw, MI", "Marquette, MI",
  "Minneapolis, MN", "Saint Paul, MN", "Rochester, MN", "Duluth, MN", "Bloomington, MN", "St. Cloud, MN", "Mankato, MN", "Moorhead, MN",
  "Charlotte, NC", "Raleigh, NC", "Durham, NC", "Chapel Hill, NC", "Greensboro, NC", "Winston-Salem, NC", "Asheville, NC", "Wilmington, NC", "Fayetteville, NC", "Cary, NC", "Boone, NC", "Greenville, NC", "High Point, NC",
  "Columbus, OH", "Cleveland, OH", "Cincinnati, OH", "Toledo, OH", "Akron, OH", "Dayton, OH", "Youngstown, OH", "Athens, OH", "Canton, OH", "Kent, OH", "Oxford, OH", "Bowling Green, OH",
  "Nashville, TN", "Memphis, TN", "Knoxville, TN", "Chattanooga, TN", "Murfreesboro, TN", "Franklin, TN", "Clarksville, TN", "Johnson City, TN",
  "Indianapolis, IN", "Bloomington, IN", "Fort Wayne, IN", "South Bend, IN", "West Lafayette, IN", "Evansville, IN", "Muncie, IN", "Carmel, IN",
  "Milwaukee, WI", "Madison, WI", "Green Bay, WI", "Kenosha, WI", "Eau Claire, WI", "Appleton, WI", "Oshkosh, WI", "La Crosse, WI",
  "Kansas City, MO", "Saint Louis, MO", "Springfield, MO", "Columbia, MO", "Independence, MO", "Jefferson City, MO", "Joplin, MO",
  "Las Vegas, NV", "Reno, NV", "Henderson, NV", "Carson City, NV", "Sparks, NV",
  "Salt Lake City, UT", "Provo, UT", "Ogden, UT", "Park City, UT", "St. George, UT", "Logan, UT",
  "Portland, ME", "Bangor, ME", "Augusta, ME", "Orono, ME",
  "Providence, RI", "Newport, RI", "Warwick, RI", "Kingston, RI",
  "Richmond, VA", "Arlington, VA", "Alexandria, VA", "Virginia Beach, VA", "Charlottesville, VA", "McLean, VA", "Reston, VA", "Norfolk, VA", "Roanoke, VA", "Blacksburg, VA", "Harrisonburg, VA", "Fairfax, VA", "Williamsburg, VA", "Chesapeake, VA", "Newport News, VA", "Lynchburg, VA", "Winchester, VA",
  "New Orleans, LA", "Baton Rouge, LA", "Shreveport, LA", "Lafayette, LA", "Lake Charles, LA", "Monroe, LA",
  "Louisville, KY", "Lexington, KY", "Bowling Green, KY", "Frankfort, KY", "Owensboro, KY",
  "Oklahoma City, OK", "Tulsa, OK", "Norman, OK", "Stillwater, OK", "Edmond, OK",
  "Omaha, NE", "Lincoln, NE", "Kearney, NE",
  "Albuquerque, NM", "Santa Fe, NM", "Las Cruces, NM", "Roswell, NM",
  "Birmingham, AL", "Montgomery, AL", "Huntsville, AL", "Tuscaloosa, AL", "Mobile, AL", "Auburn, AL", "Florence, AL",
  "Des Moines, IA", "Cedar Rapids, IA", "Iowa City, IA", "Ames, IA", "Davenport, IA", "Waterloo, IA",
  "Little Rock, AR", "Fayetteville, AR", "Bentonville, AR", "Jonesboro, AR",
  "Boise, ID", "Idaho Falls, ID", "Moscow, ID", "Coeur d'Alene, ID",
  "Newark, NJ", "Jersey City, NJ", "Princeton, NJ", "Trenton, NJ", "Hoboken, NJ", "New Brunswick, NJ", "Atlantic City, NJ", "Camden, NJ", "Paterson, NJ", "Morristown, NJ", "Edison, NJ",
  "Wilmington, DE", "Newark, DE", "Dover, DE",
  "Hartford, CT", "New Haven, CT", "Stamford, CT", "Bridgeport, CT", "Storrs, CT", "New London, CT", "Norwalk, CT", "Danbury, CT",
  "Manchester, NH", "Concord, NH", "Durham, NH", "Nashua, NH", "Portsmouth, NH", "Hanover, NH",
  "Burlington, VT", "Montpelier, VT", "Brattleboro, VT",
  "Charleston, SC", "Columbia, SC", "Greenville, SC", "Myrtle Beach, SC", "Clemson, SC", "Rock Hill, SC", "Spartanburg, SC",
  "Anchorage, AK", "Fairbanks, AK", "Juneau, AK",
  "Honolulu, HI", "Hilo, HI", "Kailua, HI", "Maui, HI",
  "Jackson, MS", "Hattiesburg, MS", "Oxford, MS", "Gulfport, MS", "Starkville, MS",
  "Charleston, WV", "Morgantown, WV", "Huntington, WV", "Wheeling, WV",
  "Fargo, ND", "Bismarck, ND", "Grand Forks, ND",
  "Sioux Falls, SD", "Rapid City, SD", "Brookings, SD",
  "Billings, MT", "Missoula, MT", "Bozeman, MT", "Helena, MT",
  "Cheyenne, WY", "Jackson, WY", "Laramie, WY",
];
...
function LocationInput({ value, onChange }) {
  ...
  const matches = useMemo(() => {
    ...
    if (!q) return [];
    return US_LOCATIONS.filter((loc) => loc.toLowerCase().includes(q)).slice(0, 6);
  }, [value]);
  ...
  const highlight = (loc) => {
    const q = (value || "").trim();
    if (!q) return loc;
    const idx = loc.toLowerCase().indexOf(q.toLowerCase());
    if (idx === -1) return loc;
    return (
      <>
        {loc.slice(0, idx)}
        <span className="loc-match">{loc.slice(idx, idx + q.length)}</span>
        {loc.slice(idx + q.length)}
      </>
    );
  };
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="field-row">
            <label>
              Company <span className="req">*</span>
              <input value={draft.company} onChange={set("company")} placeholder="e.g. BDO" autoFocus />
            </label>
            <label>
              Role / title
        ...
        <div className="iv-list">
          {sorted.map((iv) => (
            <div className="iv-row" key={iv.id}>
              ...
              <button className="icon-btn" onClick={() => removeRound(iv.id)} title="Remove round">
                <Trash2 size={13} />
              </button>
            </div>
          ))}
...
.header-actions { display: flex; align-items: center; gap: 10px; position: relative; z-index: 1; flex-wrap: wrap; }
.view-tabs {
