...
function InternlyApp() {
  ...
  const sendProgressReportNow = useCallback(async () => {
    const session = await sbGetValidSession();
    if (!session?.access_token) {
      return { success: false, message: "You're not signed in. Refresh the page and try again." };
    }
    ...
    try {
      const res = await fetch(`${SUPABASE_URL}/functions/v1/send-progress-report`, {
        ...
        body: JSON.stringify({ parentEmail: parentEmail.trim(), studentName: account?.name || "Your student" }),
      });
      let data = null;
      try {
        data = await res.json();
  ...
  const currentSettingsRef = useCallback(() => ({
    logoId, favoriteGames, showAllFilter, highContrastNav, parentEmail, reportFrequency,
  }), [logoId, favoriteGames, showAllFilter, highContrastNav, parentEmail, reportFrequency]);
  ...
  return (
    <div className={`internly-root ${highContrastNav ? "high-contrast-nav" : ""}`}>
      <style>{CSS}</style>
...
// Ko-fi's widget script manipulates the DOM directly (not React-friendly),
// so raw <script> tags in JSX won't execute it. This injects the script
// once, the first time the Dashboard is visited, and lets Ko-fi's own
// small floating button render itself - it's a corner bubble by design,
// not something that covers page content.
let kofiScriptLoaded = false;
function KofiWidget() {
  useEffect(() => {
    if (kofiScriptLoaded) return;
    ...
    script.onload = () => {
      if (window.kofiwidget2) {
        window.kofiwidget2.init("Keep me caffeinated", "#fb7b39", "P5P2DJ7ZK");
...
function SettingsPage({ logoId, onChangeLogo, account, onUpdateAccount, onSignOut, onDeleteAccount, gpaSettings, onUpdateGpaSettings, showAllFilter, onSetShowAllFilter, highContrastNav, onSetHighContrastNav, parentEmail, onSetParentEmail, reportFrequency, onSetReportFrequency, onSendProgressReportNow }) {
  ...
  const handleSendReportNow = async () => {
    ...
    setReportError("");
    setReportSent(false);
    const result = await onSendProgressReportNow();
    ...
    <div className="settings-page">
      ...
      {typeof reportFrequency === "string" && onSetReportFrequency && (
        ...
        <div className="settings-section">
          ...
          <p className="settings-sub">
            Send a summary of new applications, GPA, and scholarship activity to a parent's email, automatically.
          </p>
          <label className="field-full">
            Parent email
            <input
          ...
          <label className="field-full">
            Send automatically
            <select value={reportFrequency} onChange={(e) => onSetReportFrequency(e.target.value)}>
          ...
          <div className="settings-actions">
            <button className="btn btn-ghost" onClick={handleSendReportNow} disabled={sendingReport || !parentEmail.trim()}>
              {reportSent ? <Check size={14} /> : <Mail size={14} />}
              {sendingReport ? "Sending…" : reportSent ? "Sent" : "Send a report now"}
            </button>
            {parentEmailSaved && <span className="settings-sub" style={{ marginLeft: 8 }}>Saved</span>}
          </div>
        </div>
        </>
      )}
