...
function InternlyApp() {
  ...
  const [boardViewAll, setBoardViewAllState] = useState(true);
    ...
    (async () => {
      try {
        ...
        if (!cancelled && settings) {
          ...
          if (typeof settings.boardViewAll === "boolean") {
            setBoardViewAllState(settings.boardViewAll);
          }
        }
      } catch (e) {
        // keep defaults
  ...
  const changeLogo = useCallback(async (id) => {
    setLogoId(id);
    try {
      await sbSetData("settings", { logoId: id, favoriteGames, boardViewAll });
    } catch (e) {
      // non-fatal - logo choice just won't persist
    }
  }, [favoriteGames, boardViewAll]);
  ...
  const toggleFavoriteGame = useCallback((title) => {
    setFavoriteGames((current) => {
      ...
      sbSetData("settings", { logoId, favoriteGames: next, boardViewAll }).catch(() => {});
      return next;
    });
  }, [logoId, boardViewAll]);

  const setBoardViewAll = useCallback((value) => {
    setBoardViewAllState(value);
    sbSetData("settings", { logoId, favoriteGames, boardViewAll: value }).catch(() => {});
      ...
      <main className="content">
        ...
        {loading ? (
          ...
          <SettingsPage
            ...
            boardViewAll={boardViewAll}
            onSetBoardViewAll={setBoardViewAll}
          />
        ) : section === "academics" ? (
          <AcademicsPage
            semesters={semesters}
...
function SettingsPage({ logoId, onChangeLogo, account, onUpdateAccount, onSignOut, onDeleteAccount, gpaSettings, onUpdateGpaSettings, boardViewAll, onSetBoardViewAll }) {
  ...
  return (
    <div className="settings-page">
      ...
      {typeof boardViewAll === "boolean" && onSetBoardViewAll && (
        <div className="settings-section">
          <h3 className="settings-title">Internships display</h3>
          <p className="settings-sub">Choose how the "All" filter on the Internships page looks.</p>
          <ToggleSwitch
            checked={boardViewAll}
            onChange={() => onSetBoardViewAll(!boardViewAll)}
            label="Board view for All"
            description="Columns by status. Turn off for a simple scrolling list instead."
          />
        </div>
      )}
